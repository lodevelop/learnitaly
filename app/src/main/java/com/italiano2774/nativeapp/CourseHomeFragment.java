...
public class CourseHomeFragment extends Fragment {
    ...
    @Nullable @Override public View onCreateView(@NonNull LayoutInflater inflater,@Nullable ViewGroup container,@Nullable Bundle state){
        ...
        v.findViewById(R.id.button_home_quick_words).setOnClickListener(x->((MainActivity)requireActivity()).openSmartMemory());
        v.findViewById(R.id.button_home_quick_calendar).setOnClickListener(x->((MainActivity)requireActivity()).openReviewCalendar());
        String requested=getArguments()==null?null:getArguments().getString(ARG_UNIT);detailsOpen=state!=null?state.getBoolean("home_details",false):!progress.homeSimpleMode()||requested!=null;detailsReady=false;detailsToggle.setOnClickListener(x->{detailsOpen=!detailsOpen;applyDetailVisibility();});applyDetailVisibility();CourseUnit candidate=curriculum.byId(requested);unit=candidate!=null&&curriculum.isUnlocked(candidate,progress)?candidate:curriculum.current(progress);render();skipInitialResume=true;return v;
    }

    @Override public void onResume(){super.onResume();if(skipInitialResume){skipInitialResume=false;return;}if(progress!=null&&unit!=null){CourseUnit current=curriculum.current(progress);String requested=getArguments()==null?null:getArguments().getString(ARG_UNIT);if(requested==null&&current!=null)unit=current;render();}}

    private void render(){if(unit==null)return;int done=curriculum.completedLessons(unit,progress);boolean complete=curriculum.isComplete(unit,progress);stage.setText(LearningLanguage.ui(progress,CourseCurriculumRepository.stageName(unit.stage)+" · 第"+unit.stageUnit+"单元",LearningLanguage.stageName(unit.stage,progress)+" · Unit "+unit.stageUnit));title.setText(LearningLanguage.courseTitle(unit,progress));CourseLessonEngine pathLabels=new CourseLessonEngine(words,progress);if(BeginnerGuideEngine.active(progress))subtitle.setText(LearningLanguage.ui(progress,"第"+BeginnerGuideEngine.day(progress)+"个学习日 · 今天不用研究全部功能，只跟着“继续学习”往下走。","Study day "+BeginnerGuideEngine.day(progress)+" · Just follow Continue learning; you do not need to explore every feature today."));else subtitle.setText(LearningLanguage.ui(progress,"当前单元 · "+pathLabels.pathSummary(unit)+" · 听、说、读、写会由系统自动穿插。","Current unit · "+pathLabels.pathSummary(unit)+" · Listening, speaking, reading and writing are mixed in automatically."));unitProgress.setProgress((int)Math.round(done*100.0/Math.max(1,unit.lessonCount)));streak.setText(LearningLanguage.ui(progress,"🔥 "+progress.activityStreak()+"天","🔥 "+progress.activityStreak()+" days"));xp.setText("⭐ "+progress.courseXp());renderDailyPlan();long activeMin=Math.round(progress.dailyActiveSeconds(java.time.LocalDate.now())/60.0);int remain=dailyPlan==null?progress.sessionMinutes():dailyPlan.remainingMinutes();daily.setText(LearningLanguage.ui(progress,"今天目标 "+progress.sessionMinutes()+" 分钟 · 已学习 "+activeMin+" 分钟 · 计划还剩约 "+remain+" 分钟","Today: "+progress.sessionMinutes()+" min · Studied "+activeMin+" min · About "+remain+" min left"));detailsReady=false;if(detailsOpen)renderDetails();
        CourseUnit next=curriculum.byIndex(unit.index+1);nextUnit.setText(next==null?LearningLanguage.ui(progress,"你已经走到完整课程的最后一站","You have reached the final course unit"):LearningLanguage.ui(progress,"下一单元：","Next unit: ")+LearningLanguage.courseTitle(next,progress));
        int lesson=curriculum.firstIncompleteLesson(unit,progress);if(complete){CourseUnit c=curriculum.current(progress);boolean hasNext=c!=null&&c.index!=unit.index;continueButton.setText(hasNext?LearningLanguage.ui(progress,"继续下一单元","Continue to next unit"):LearningLanguage.ui(progress,"复习这个单元","Review this unit"));continueButton.setOnClickListener(x->{if(hasNext)((MainActivity)requireActivity()).openCourseUnit(c.id);else ((MainActivity)requireActivity()).openCourseLesson(unit.id,Math.max(0,unit.lessonCount-1));});}
        else{continueButton.setText(done>0?LearningLanguage.ui(progress,"继续学习 · 第"+(lesson+1)+"关","Continue learning · Stage "+(lesson+1)):LearningLanguage.ui(progress,"开始第1关","Start stage 1"));continueButton.setOnClickListener(x->((MainActivity)requireActivity()).openCourseLesson(unit.id,lesson));}
    }


    @Override public void onSaveInstanceState(@NonNull Bundle state){super.onSaveInstanceState(state);state.putBoolean("home_details",detailsOpen);}

    private void renderDetails(){if(unit==null||detailsReady)return;renderSmartMemory();renderMemoryArticles();renderPath(curriculum.completedLessons(unit,progress),curriculum.isComplete(unit,progress));detailsReady=true;}

    private void applyDetailVisibility(){if(homeDetails==null||detailsToggle==null)return;homeDetails.setVisibility(detailsOpen?View.VISIBLE:View.GONE);if(detailsOpen)renderDetails();detailsToggle.setText(detailsOpen?LearningLanguage.ui(progress,"收起今日详情 ▴","Hide today\'s details ▴"):LearningLanguage.ui(progress,"查看今日详情 ▾","View today\'s details ▾"));}
    ...
    private void renderDailyPlan(){
        ...
        dailyPlanProgress.setProgress(dailyPlan.progressPercent());dailyPlanSummary.setText(LearningLanguage.ui(progress,done+" / "+total+"项完成 · 还剩约"+remaining+"分钟 · 今日目标"+dailyPlan.targetMinutes+"分钟",done+" / "+total+" tasks complete · about "+remaining+" min left · "+dailyPlan.targetMinutes+" min goal"));
        dailyNextTask.setText(next==null?LearningLanguage.ui(progress,"🎉 今天的主任务完成了","🎉 Today\'s main tasks are complete"):(LearningLanguage.ui(progress,"下一步：","Next: ")+next.emoji+"  "+LearningLanguage.uiText(progress,next.title)+LearningLanguage.ui(progress," · 约"+next.minutes+"分钟"," · about "+next.minutes+" min")));coachHint.setText(BeginnerGuideEngine.coachLine(progress,dailyPlan,next));
        dailyPlanDetail.setText(LearningLanguage.uiText(progress,dailyPlan.focus)+(dailyPlan.recoveryMode?LearningLanguage.ui(progress,"\n今天已自动优先处理反复出错和容易遗忘的内容。","\nRepeated errors and easily forgotten items are prioritized today."):"")+(dailyPlan.breakthroughSummary==null||dailyPlan.breakthroughSummary.isEmpty()?"":LearningLanguage.ui(progress,"\n连续补弱：","\nFocused improvement: ")+LearningLanguage.uiText(progress,dailyPlan.breakthroughSummary)));
        for(int i=0;i<dailyPlan.tasks.size();i++){
            DailySmartTask t=dailyPlan.tasks.get(i);LinearLayout row=new LinearLayout(requireContext());row.setOrientation(LinearLayout.HORIZONTAL);row.setGravity(Gravity.CENTER_VERTICAL);row.setPadding(dp(2),dp(10),dp(2),dp(10));row.setMinimumHeight(dp(64));
            ...
            TextView taskTitle=new TextView(requireContext());taskTitle.setText((i+1)+". "+LearningLanguage.uiText(progress,t.title)+(t==next?LearningLanguage.ui(progress," · 下一项"," · Next"):""));taskTitle.setTextSize(15);taskTitle.setTextColor(ContextCompat.getColor(requireContext(),t==next?R.color.blue:R.color.text_primary));taskTitle.setTypeface(android.graphics.Typeface.DEFAULT,android.graphics.Typeface.BOLD);copy.addView(taskTitle);
            TextView taskSub=new TextView(requireContext());taskSub.setText(LearningLanguage.uiText(progress,t.subtitle));taskSub.setTextSize(13);taskSub.setTextColor(ContextCompat.getColor(requireContext(),R.color.text_secondary));copy.addView(taskSub);
            TextView state=new TextView(requireContext());state.setText(t.done?LearningLanguage.ui(progress,"完成","Done"):LearningLanguage.ui(progress,"约"+t.minutes+"分","about "+t.minutes+" min"));state.setTextSize(12);state.setTextColor(ContextCompat.getColor(requireContext(),t.done?R.color.success:R.color.text_secondary));row.addView(state);
            if(!t.done){row.setFocusable(true);row.setOnClickListener(x->openDailyTask(t));}dailyPlanContainer.addView(row,new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT));
        }
        if(next==null){dailyPlanButton.setText(LearningLanguage.ui(progress,"查看今日总结 · 明日复习预告","View today\'s summary · Tomorrow\'s review preview"));dailyPlanButton.setEnabled(true);dailyPlanButton.setOnClickListener(x->((MainActivity)requireActivity()).openDailySummary());}else{dailyPlanButton.setEnabled(true);dailyPlanButton.setText(LearningLanguage.ui(progress,"开始下一项 · ","Start next · ")+LearningLanguage.uiText(progress,next.title));dailyPlanButton.setOnClickListener(x->openDailyTask(next));}
    }
    ...
    private void renderSmartMemory(){int due=0,unknown=0,mastered=0,route=Math.min(progress.vocabularyRouteLimit(),words.size());java.time.LocalDate today=java.time.LocalDate.now();java.util.List<Word> all=words.all();for(int i=0;i<all.size();i++){Word w=all.get(i);int m=progress.mastery(w.id);if(m>0&&progress.dueForReview(w.id,today))due++;if(progress.smartMastered(w.id))mastered++;if(i<route&&m==0)unknown++;}int fresh=Math.min(progress.perDay(),unknown);smartSummary.setText(LearningLanguage.ui(progress,"今日复习 "+due+" · 新词 "+fresh+" · 四维掌握 "+mastered+"\n自动轮换：意→中 · 中→意 · 听音识词 · 句子填空；哪项弱就多练哪项。\n名词连冠词记 · 动词连原形/变位记 · 🇬🇧 英语近似词自动助记 · 词块继续保留","Due reviews "+due+" · New words "+fresh+" · 4-skill mastered "+mastered+"\nAutomatic rotation: Italian→meaning · meaning→Italian · listening recognition · sentence cloze; weaker skills appear more often.\nLearn nouns with articles · verbs with infinitives/forms · 🇬🇧 English cognates as memory aids · keep useful chunks"));smartButton.setEnabled(due+fresh>0);smartButton.setText(due+fresh>0?LearningLanguage.ui(progress,"开始四模式智能背词","Start 4-skill vocabulary"):LearningLanguage.ui(progress,"今日已完成","Completed today"));}


    private void renderMemoryArticles(){java.util.List<Integer> ids=new java.util.ArrayList<>();for(int id=1;id<=Math.min(2000,words.size());id++)ids.add(id);int done=progress.memoryArticleCompletedTotal(memoryArticles),total=memoryArticles.totalSections(),encountered=progress.memoryArticleEncounteredCount(ids),recognized=progress.memoryArticleRecognizedCount(ids),mastered=progress.memoryArticleMasteredCount(ids);memoryArticleSummary.setText(LearningLanguage.ui(progress,"已完成 "+done+" / "+total+" 小节 · 已遇到 "+encountered+" · 已认识 "+recognized+" · 真正掌握 "+mastered+"\n螺旋复现：每节40个新目标词，并主动带回12个旧词；新增逐句4轮背诵与正常/慢速朗读，最后继续四模式智能复习。","Completed "+done+" / "+total+" sections · Encountered "+encountered+" · Recognized "+recognized+" · Mastered "+mastered+"\nSpiral review: 40 new target words per section plus 12 older words; 4-pass sentence recall with normal/slow audio, followed by 4-skill smart review."));memoryArticleButton.setText(done>=total&&total>0?LearningLanguage.ui(progress,"十篇已完成 · 进入复习","10 stories complete · Review"):LearningLanguage.ui(progress,"进入十篇通关","Open 10-story course"));}
    ...
    private void renderPath(int done,boolean complete){path.removeAllViews();CourseLessonEngine labels=new CourseLessonEngine(words,progress);int firstIncomplete=Math.min(done,unit.lessonCount-1);for(int i=0;i<unit.lessonCount;i++){boolean migrated=unit.index<progress.courseUnlockedUnitIndex();boolean finished=migrated||progress.courseLessonDone(unit.id,i);boolean unlocked=finished||complete||i<=firstIncomplete;LinearLayout row=new LinearLayout(requireContext());row.setGravity(Gravity.CENTER_VERTICAL);row.setOrientation(LinearLayout.HORIZONTAL);LinearLayout.LayoutParams rp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);rp.setMargins(dp(10),dp(5),dp(10),dp(5));path.addView(row,rp);
            ...
            TextView label=new TextView(requireContext());label.setText(labels.lessonTitle(unit,i));label.setGravity(Gravity.CENTER);label.setTextSize(13);label.setTextColor(ContextCompat.getColor(requireContext(),unlocked?R.color.text_primary:R.color.text_secondary));LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(dp(128),ViewGroup.LayoutParams.WRAP_CONTENT);lp.topMargin=dp(4);nodeBox.addView(label,lp);
