...
public final class SmartReviewModeEngine {
    ...
    public static int choose(Word w, ProgressStore progress, WordRepository repo) {
        ...
        for (int mode : candidates) if (isAvailable(mode, w, repo, progress)) return mode;
        return MODE_IT_ZH;
    }
    ...
    public static boolean isAvailable(int mode, Word w, WordRepository repo) { return isAvailable(mode,w,repo,null); }

    public static boolean isAvailable(int mode, Word w, WordRepository repo, ProgressStore progress) {
        if (w == null) return false;
        if (mode == MODE_ZH_IT) return repo != null && (progress==null?repo.isChinesePromptUnique(w):repo.isSupportPromptUnique(w,progress));
