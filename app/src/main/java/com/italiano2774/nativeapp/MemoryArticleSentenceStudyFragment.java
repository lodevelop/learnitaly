...
public class MemoryArticleSentenceStudyFragment extends Fragment {
    ...
    private void render(){
        ...
        switch(phase){
            case 0:
                mode.setText("第1轮 · 双语理解：先把这一句真正看懂，再进入听读。");italian.setText(s.italian);italian.setVisibility(View.VISIBLE);chinese.setText(LearningLanguage.sentenceSupport(words,s.italian,s.chinese,progress));chinese.setVisibility(View.VISIBLE);next.setText("下一步 · 听读这句");break;
            case 1:
            ...
            case 2:
                mode.setText("第3轮 · 挖空回忆：先补出空缺，再显示完整句子核对。");italian.setText(revealed?s.italian:clozeSentence(s.italian));italian.setVisibility(View.VISIBLE);chinese.setText(LearningLanguage.sentenceSupport(words,s.italian,s.chinese,progress));chinese.setVisibility(View.VISIBLE);reveal.setVisibility(revealed?View.GONE:View.VISIBLE);reveal.setText("显示完整句子");next.setEnabled(revealed);next.setText(revealed?"下一步 · 中文反推":"先显示答案再继续");break;
            default:
                mode.setText("第4轮 · 中文反推：只看中文，大声说出整句意大利语，再核对。");italian.setText(revealed?s.italian:"先不要看意大利语 · 根据下面中文大声说出整句");italian.setVisibility(View.VISIBLE);chinese.setText(LearningLanguage.sentenceSupport(words,s.italian,s.chinese,progress));chinese.setVisibility(View.VISIBLE);reveal.setVisibility(revealed?View.GONE:View.VISIBLE);reveal.setText("显示意大利语答案");next.setEnabled(revealed);next.setText(revealed?(sentenceIndex+1<section.sentences.size()?"下一句":"完成逐句背诵"):"先显示答案再继续");break;
        }
    }
    ...
    private String keyWordsText(String sentence){
        ...
        for(Integer id:section.targetWordIds){if(id==null)continue;Word w=words.byId(id);if(w==null||w.word==null||w.word.trim().isEmpty())continue;String token=w.word.trim();if(containsWord(lower,token.toLowerCase(Locale.ITALIAN))){found.add(token+"  "+LearningLanguage.wordMeaning(w,progress));if(found.size()>=6)break;}}
