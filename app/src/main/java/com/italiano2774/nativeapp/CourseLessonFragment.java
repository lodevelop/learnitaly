...
public class CourseLessonFragment extends Fragment {
    ...
    private android.widget.ScrollView lessonScroll;private View accentsScroll;private MaterialButton dontKnow;private TextView questionCount;
    ...
    private void bind(View v){lessonScroll=v.findViewById(R.id.scroll_course_lesson);accentsScroll=v.findViewById(R.id.scroll_course_accents);questionCount=v.findViewById(R.id.text_course_question_count);dontKnow=v.findViewById(R.id.button_course_dont_know);
        dontKnow.setOnClickListener(x->{CourseQuestion q=current();if(q!=null&&!checked&&!teachingMeaning&&q.type!=CourseQuestion.INTRO)evaluate(q,true);});
        LinearLayout accents=v.findViewById(R.id.container_course_accents);
        for(String letter:new String[]{"à","è","é","ì","ò","ù","’"}){
            MaterialButton key=new MaterialButton(requireContext());key.setText(letter);key.setAllCaps(false);key.setMinWidth(0);key.setTextSize(18);key.setContentDescription(letter);
            key.setOnClickListener(x->{if(input==null||!input.isEnabled()||input.getText()==null)return;int start=Math.max(0,input.getSelectionStart()),end=Math.max(0,input.getSelectionEnd());int lo=Math.min(start,end),hi=Math.max(start,end);input.getText().replace(lo,hi,letter);input.setSelection(lo+letter.length());input.requestFocus();});
            accents.addView(key,new LinearLayout.LayoutParams(dp(48),dp(48)));
        }
        bar=v.findViewById(R.id.progress_course_lesson);xp=v.findViewById(R.id.text_course_lesson_xp);header=v.findViewById(R.id.text_course_lesson_header);prompt=v.findViewById(R.id.text_course_lesson_prompt);display=v.findViewById(R.id.text_course_lesson_display);support=v.findViewById(R.id.text_course_lesson_support);hint=v.findViewById(R.id.text_course_hint);feedback=v.findViewById(R.id.text_course_feedback);finishTitle=v.findViewById(R.id.text_course_finish_title);finishSummary=v.findViewById(R.id.text_course_finish_summary);choices=v.findViewById(R.id.container_course_choices);finishPanel=v.findViewById(R.id.panel_course_finish);inputLayout=v.findViewById(R.id.layout_course_answer);input=v.findViewById(R.id.input_course_answer);feedbackCard=v.findViewById(R.id.card_course_feedback);audioButton=v.findViewById(R.id.button_course_lesson_audio);audioButton.setContentDescription(LearningLanguage.ui(progress,"播放意大利语发音","Play Italian pronunciation"));v.findViewById(R.id.button_course_lesson_close).setContentDescription(LearningLanguage.ui(progress,"返回学习路径","Return to learning path"));checkButton=v.findViewById(R.id.button_course_check);v.findViewById(R.id.button_course_lesson_close).setOnClickListener(x->close());audioButton.setOnClickListener(x->{CourseQuestion q=current();if(q!=null&&q.word!=null)audio.play(q.word);});checkButton.setOnClickListener(x->onAction());}
    ...
    private void render(){
        ...
        dontKnow.setVisibility(!teachingMeaning&&q.type!=CourseQuestion.INTRO?View.VISIBLE:View.GONE);accentsScroll.setVisibility(inputLayout.getVisibility()==View.VISIBLE?View.VISIBLE:View.GONE);
    ...
    private void renderChoices(CourseQuestion q){for(String opt:q.options){MaterialButton b=new MaterialButton(requireContext());b.setAllCaps(false);b.setText(opt);b.setTextSize(16);b.setMinHeight(dp(56));b.setPadding(dp(16),dp(12),dp(16),dp(12));b.setGravity(android.view.Gravity.START|android.view.Gravity.CENTER_VERTICAL);b.setBackgroundTintList(ColorStateList.valueOf(ContextCompat.getColor(requireContext(),R.color.surface)));b.setTextColor(ContextCompat.getColor(requireContext(),R.color.text_primary));b.setStrokeColor(ColorStateList.valueOf(ContextCompat.getColor(requireContext(),R.color.line)));b.setStrokeWidth(dp(1));b.setCornerRadius(dp(14));b.setOnClickListener(x->{if(checked)return;selectedOption=opt;for(int i=0;i<choices.getChildCount();i++){View c=choices.getChildAt(i);if(c instanceof MaterialButton){MaterialButton mb=(MaterialButton)c;boolean sel=mb==b;mb.setStrokeColor(ColorStateList.valueOf(ContextCompat.getColor(requireContext(),sel?R.color.blue:R.color.line)));mb.setStrokeWidth(dp(sel?2:1));mb.setSelected(sel);mb.setBackgroundTintList(ColorStateList.valueOf(ContextCompat.getColor(requireContext(),sel?R.color.home_accent_surface:R.color.surface)));}}checkButton.setEnabled(true);});LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);lp.bottomMargin=dp(8);choices.addView(b,lp);}}

    private void onAction(){if(finished){closeToPath();return;}CourseQuestion q=current();if(q==null){showFinish();return;}if(teachingMeaning&&!checked){meaningPreviewPassedIndex=index;teachingMeaning=false;render();return;}if(q.type==CourseQuestion.INTRO&&!checked){index++;saveResume();render();return;}if(!checked){evaluate(q);return;}index++;checked=false;saveResume();render();}
    private void evaluate(CourseQuestion q){evaluate(q,false);}
    private void evaluate(CourseQuestion q,boolean giveUp){if(checked)return;String actual=isChoice(q)?selectedOption:(input.getText()==null?"":input.getText().toString());if(giveUp)actual="";if(!giveUp&&!isChoice(q)&&actual.trim().isEmpty()){input.setError(LearningLanguage.ui(progress,"先输入答案；如果暂时不会，可以返回后在复习里再练","Enter an answer first. If you do not know it yet, return and review it later."));return;}boolean ok=CourseAttemptPolicy.isCorrect(giveUp,equalsAnswer(actual,q.answer));checked=true;if(ok)correct++;else wrong++;long ms=Math.max(0,SystemClock.elapsedRealtime()-shownAt);if(q.type!=CourseQuestion.INTRO&&q.word!=null){if(q.type==CourseQuestion.ACTIVE)progress.recordDimensionResults(q.word.id,new int[]{ProgressStore.DIM_MEANING,ProgressStore.DIM_SPELLING},ok,ms);else if(q.type!=CourseQuestion.GRAMMAR_ARTICLE&&q.type!=CourseQuestion.GRAMMAR_VERB)progress.recordDimensionResult(q.word.id,q.dimension,ok,ms);if(!ok){String cause=q.type==CourseQuestion.LISTEN?ErrorCause.LISTENING_CONFUSION:(q.type==CourseQuestion.GRAMMAR_ARTICLE?ErrorCause.ARTICLE_GENDER:(q.type==CourseQuestion.GRAMMAR_VERB?ErrorCause.WORD_FORM:(q.type==CourseQuestion.SPELL_HINT||q.type==CourseQuestion.ACTIVE?ErrorCause.SPELLING:ErrorCause.RECALL)));progress.recordErrorCause(cause,q.word.id,"course",q.answer,actual,CourseCurriculumRepository.stageName(unit.stage)+" · "+unit.titleZh+" · 第"+(lesson+1)+"课");}}
        feedbackCard.setVisibility(View.VISIBLE);feedback.setText(ok?"✓ 正确":"答案："+q.answer+"\n没关系，本单元后续巩固或智能复习还会再遇到它。");feedback.setTextColor(ContextCompat.getColor(requireContext(),ok?R.color.success:R.color.error));checkButton.setText("继续");checkButton.setEnabled(true);dontKnow.setVisibility(View.GONE);accentsScroll.setVisibility(View.GONE);disableInputs();hideKeyboard();saveResume();feedbackCard.post(()->{if(isAdded()&&checked)lessonScroll.smoothScrollTo(0,feedbackCard.getTop());});}
