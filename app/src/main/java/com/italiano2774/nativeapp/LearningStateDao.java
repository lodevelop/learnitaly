...
@Dao public interface LearningStateDao{
 ...
 @Query("SELECT e.* FROM error_records e WHERE e.id IN (SELECT MAX(id) FROM error_records WHERE repairedAt=0 AND expected!='' AND (mode IN ('writing','freechat','daily_speaking','intensive_listening','listening_course','sentence_dictation','shadowing','dialogue_beginner','dialogue_intermediate','dialogue_advanced','listen_speak_output','weekly_exam_output') OR (mode='listen_speak' AND wordId=0 AND detail LIKE '%语音识别内容匹配%') OR (mode='weekly_exam' AND wordId=0 AND (detail LIKE '周测口语%' OR detail LIKE '周测真实使用%'))) GROUP BY mode,expected,actual) ORDER BY e.createdAt DESC LIMIT :limit") List<ErrorRecordEntity> unresolvedPracticeErrors(int limit);
 @Query("SELECT COUNT(*) FROM (SELECT 1 FROM error_records WHERE repairedAt=0 AND expected!='' AND (mode IN ('writing','freechat','daily_speaking','intensive_listening','listening_course','sentence_dictation','shadowing','dialogue_beginner','dialogue_intermediate','dialogue_advanced','listen_speak_output','weekly_exam_output') OR (mode='listen_speak' AND wordId=0 AND detail LIKE '%语音识别内容匹配%') OR (mode='weekly_exam' AND wordId=0 AND (detail LIKE '周测口语%' OR detail LIKE '周测真实使用%'))) GROUP BY mode,expected,actual)") int unresolvedPracticeErrorCount();
 ...
 @Query("UPDATE error_records SET repairedAt=:whenMs WHERE id=:id AND repairedAt=0") int archiveUnrepairableError(long id,long whenMs);
