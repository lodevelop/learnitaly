...
public class DialogueTrainingFragment extends Fragment {
    ...
    private void revealMeaningOrAnswer(){
        ...
        phase=PHASE_UNDERSTAND;stage.setText("② 看懂这句");npc.setText(t.npc);npcZh.setText(LearningLanguage.sentenceSupport(words,t.npc,t.npcZh,progress));npcZh.setVisibility(View.VISIBLE);reveal.setVisibility(View.GONE);startAnswer.setVisibility(View.VISIBLE);
    ...
    private void showSpeakPhase(){
        ...
        if(currentLevel==LEVEL_ADVANCED){
            ...
            speakPrompt.setText(t.answerZh==null||t.answerZh.trim().isEmpty()?LearningLanguage.ui(progress,"把你的回答直接说出来","Give your answer in Italian"):LearningLanguage.sentenceSupport(words,correctAnswer(t),t.answerZh,progress));target.setText("只看中文，自己组织意大利语。允许自然的同义表达。");hint.setText("看意大利语提示");
        }else{
            speakPrompt.setText(t.answerZh==null||t.answerZh.trim().isEmpty()?LearningLanguage.ui(progress,"把刚才的正确回答自己说出来","Say the correct response yourself"):LearningLanguage.sentenceSupport(words,correctAnswer(t),t.answerZh,progress));target.setText("先自己想，不显示意大利语");hint.setText("看提示");
        }
    ...
    private void showTargetHint(){
        ...
        if(currentLevel==LEVEL_ADVANCED&&turnHintLevel==1){
            target.setText(LearningLanguage.ui(progress,"提示1 · 对方的意思：","Hint 1 · The other person means: ")+LearningLanguage.sentenceSupport(words,t.npc,t.npcZh,progress)+"\n"+LearningLanguage.ui(progress,"你可以表达：","You can express: ")+LearningLanguage.sentenceSupport(words,correctAnswer(t),t.answerZh,progress)+"\n"+LearningLanguage.ui(progress,"再试着自己说，不显示意大利语。","Try again without showing the Italian answer."));
            hint.setText("再看参考表达");
        }else{
    ...
    private void scoreSpeech(List<String> hypotheses){
        ...
        if(!ok)progress.recordErrorCause(currentLevel==LEVEL_BEGINNER?ErrorCause.PRONUNCIATION:ErrorCause.RECALL,0,"dialogue_"+levelKey(),expected,best,ErrorEvidencePolicy.detailWithPrompt(current.title+" · 匹配/理解 "+bestScore+"%",LearningLanguage.sentenceSupport(words,expected,t.answerZh,progress)));
    ...
    private void showReply(boolean spoken){
        ...
        speakContainer.setVisibility(View.GONE);npc.setText(t.npc);npcZh.setText(LearningLanguage.sentenceSupport(words,t.npc,t.npcZh,progress));npcZh.setVisibility(View.VISIBLE);
