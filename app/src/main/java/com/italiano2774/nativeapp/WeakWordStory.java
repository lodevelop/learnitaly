...
public class WeakWordStory {
    ...
    public String targetSummary(ProgressStore progress){StringBuilder sb=new StringBuilder();for(Word w:targetWords){if(sb.length()>0)sb.append("  ·  ");sb.append(w.word);String meaning=LearningLanguage.wordMeaning(w,progress);if(!meaning.isEmpty())sb.append(" ").append(meaning);}return sb.toString();}
    public String targetSummary(){return targetSummary(null);}
}
