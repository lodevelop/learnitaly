...
public class ReadingAdapter extends RecyclerView.Adapter<ReadingAdapter.H>{
    ...
    public void onBindViewHolder(@NonNull H h,int pos){ReadingPassage r=items.get(pos);h.level.setText(r.level);h.title.setText(r.title);h.zh.setText(LearningLanguage.titleSupport(WordRepository.get(h.itemView.getContext()),r.title,r.titleZh,progress));int best=progress.readingBest(r.id);ReadingComprehensibilityEngine.Score sc=engine.score(r);h.status.setText("已知 "+sc.coverage+"% · "+sc.label()+(best>0?(" · 最好 "+best+"%"):""));h.itemView.setOnClickListener(v->listener.open(r));}
