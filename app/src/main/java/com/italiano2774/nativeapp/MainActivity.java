...
public class MainActivity extends AppCompatActivity {
    ...
    @Override protected void onCreate(Bundle s){
        ...
        LearningLanguage.applyBottomNav(nav,progress);
        getSupportFragmentManager().registerFragmentLifecycleCallbacks(new FragmentManager.FragmentLifecycleCallbacks(){
            @Override public void onFragmentViewCreated(FragmentManager fm,Fragment f,View v,Bundle state){v.post(()->LearningLanguage.watchUi(v,progress));}
            @Override public void onFragmentResumed(FragmentManager fm,Fragment f){View v=f.getView();if(v!=null)v.post(()->LearningLanguage.applyUi(v,progress));}
        },true);
    ...
    public void openReviewCalendar(){BottomNavigationView nav=findViewById(R.id.bottom_navigation);nav.getMenu().findItem(R.id.nav_calendar).setChecked(true);show(new CalendarFragment());}
    ...
    public void openMemoryArticles(){BottomNavigationView nav=findViewById(R.id.bottom_navigation);nav.getMenu().findItem(R.id.nav_today).setChecked(true);show(new MemoryArticleListFragment());}
    public void openMemoryArticle(String articleId){BottomNavigationView nav=findViewById(R.id.bottom_navigation);nav.getMenu().findItem(R.id.nav_today).setChecked(true);show(MemoryArticleDetailFragment.newInstance(articleId));}
    public void openMemoryArticleStudy(String articleId,int sectionIndex){BottomNavigationView nav=findViewById(R.id.bottom_navigation);nav.getMenu().findItem(R.id.nav_today).setChecked(true);show(MemoryArticleStudyFragment.newInstance(articleId,sectionIndex));}
    public void openMemoryArticleSentenceStudy(String articleId,int sectionIndex){BottomNavigationView nav=findViewById(R.id.bottom_navigation);nav.getMenu().findItem(R.id.nav_today).setChecked(true);show(MemoryArticleSentenceStudyFragment.newInstance(articleId,sectionIndex));}
    public void openMemoryArticleReview(int[] wordIds,String label,int maxCards){BottomNavigationView nav=findViewById(R.id.bottom_navigation);nav.getMenu().findItem(R.id.nav_today).setChecked(true);show(StudySessionFragment.newArticleReviewInstance(wordIds,label,maxCards));}
    ...
    public void openListeningSpeaking(){BottomNavigationView nav=findViewById(R.id.bottom_navigation);nav.getMenu().findItem(R.id.nav_practice).setChecked(true);show(new ListeningSpeakingFragment());}
    public void openDailySpeakingChallenge(){BottomNavigationView nav=findViewById(R.id.bottom_navigation);nav.getMenu().findItem(R.id.nav_practice).setChecked(true);show(new DailySpeakingChallengeFragment());}
    ...
    public void openLifeTaskMap(){BottomNavigationView nav=findViewById(R.id.bottom_navigation);nav.getMenu().findItem(R.id.nav_practice).setChecked(true);show(new LifeTaskMapFragment());}
    public void openLifeTask(String taskId){BottomNavigationView nav=findViewById(R.id.bottom_navigation);nav.getMenu().findItem(R.id.nav_practice).setChecked(true);show(LifeTaskDetailFragment.newInstance(taskId));}
    public void openLifeTaskStage(String taskId,int level){LifeTask task=LifeTaskRepository.find(taskId);if(task==null){openLifeTaskMap();return;}BottomNavigationView nav=findViewById(R.id.bottom_navigation);nav.getMenu().findItem(R.id.nav_practice).setChecked(true);show(DialogueTrainingFragment.newInstance(task.scenarioId,level,task.id));}
    ...
    public void openMicroGrammar(){BottomNavigationView nav=findViewById(R.id.bottom_navigation);nav.getMenu().findItem(R.id.nav_practice).setChecked(true);show(new MicroGrammarFragment());}
    public void openMicroGrammarLesson(String patternId){BottomNavigationView nav=findViewById(R.id.bottom_navigation);nav.getMenu().findItem(R.id.nav_practice).setChecked(true);show(SentencePatternFragment.newMicroInstance(patternId));}
    ...
    public void openWeakWordStory(){openWeakWordStory(0);}
    public void openWeakWordStory(int variant){BottomNavigationView nav=findViewById(R.id.bottom_navigation);nav.getMenu().findItem(R.id.nav_practice).setChecked(true);show(WeakWordStoryFragment.newInstance(variant));}
    ...
    public void openWeeklyExam(){BottomNavigationView nav=findViewById(R.id.bottom_navigation);nav.getMenu().findItem(R.id.nav_practice).setChecked(true);show(new WeeklyExamFragment());}
    ...
    public void openErrorEvidenceRepair(){BottomNavigationView nav=findViewById(R.id.bottom_navigation);nav.getMenu().findItem(R.id.nav_practice).setChecked(true);show(new ErrorEvidenceRepairFragment());}
    ...
    public void openMasteryPassport(){BottomNavigationView nav=findViewById(R.id.bottom_navigation);nav.getMenu().findItem(R.id.nav_settings).setChecked(true);show(new MasteryPassportFragment());}
    public void openForgettingProfile(){BottomNavigationView nav=findViewById(R.id.bottom_navigation);nav.getMenu().findItem(R.id.nav_settings).setChecked(true);show(new ForgettingProfileFragment());}
    ...
    private void reconcilePendingErrorRepairs(){final Context app=getApplicationContext();new Thread(()->{try{int n=LearningDatabase.get(app).learningStateDao().unresolvedPracticeErrorCount();progress.setPendingErrorRepairs(n);}catch(Exception e){LocalErrorLog.write(app,"v5.0.2 pending sentence-evidence reconcile",e);}},"error-evidence-reconcile").start();}
    ...
    public void exportProgress(){
        ...
        new Thread(()->{
            try{
                ...
                root.put("customItems",custom);JSONArray errors=new JSONArray();for(ErrorRecordEntity er:LearningDatabase.get(this).learningStateDao().recentErrors(1000)){JSONObject o=new JSONObject();o.put("createdAt",er.createdAt);o.put("wordId",er.wordId);o.put("mode",er.mode);o.put("cause",er.cause);o.put("expected",er.expected);o.put("actual",er.actual);o.put("detail",er.detail);o.put("repairedAt",er.repairedAt);o.put("repairAttempts",er.repairAttempts);errors.put(o);}root.put("errorHistory",errors);LocalBackupManager.appendAdvancedProgress(this,root);String json=root.toString(2);
                runOnUiThread(()->{Intent i=new Intent(Intent.ACTION_CREATE_DOCUMENT);i.addCategory(Intent.CATEGORY_OPENABLE);i.setType("application/json");i.putExtra(Intent.EXTRA_TITLE,"终学意语_backup_v5_0.json");pending=json;startActivityForResult(i,EXPORT);});
