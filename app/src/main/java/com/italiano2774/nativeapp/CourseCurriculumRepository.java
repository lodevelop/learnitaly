...
public class CourseCurriculumRepository {
    ...
    private final Map<Integer,CourseUnit> byWordId=new HashMap<>();
    ...
    private CourseCurriculumRepository(Context context){
        try{
            ...
            for(int i=0;i<arr.length();i++){
                ...
                JSONArray ids=o.getJSONArray("wordIds");for(int j=0;j<ids.length();j++){int wid=ids.getInt(j);if(byWordId.containsKey(wid))throw new IllegalStateException("Duplicate fixed-course word owner: "+wid+" in "+byWordId.get(wid).id+" and "+u.id);u.wordIds.add(wid);byWordId.put(wid,u);}
    ...
    public CourseUnit ownerOfWord(int wordId){return byWordId.get(wordId);}
