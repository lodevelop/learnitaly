...
public class WordAdapter extends RecyclerView.Adapter<WordAdapter.Holder> {
    ...
    private final Context context;private final ProgressStore progress;private final AudioPlayer audio;private final EnglishBridgeRepository bridgeRepo;private final MemoryAidRepository memoryAidRepo;private final ChangeListener listener;
    private final List<Word> items=new ArrayList<>();
    private final Set<Integer> expanded=new HashSet<>();
    private final List<String> masteryLabels;

    public WordAdapter(Context c,ProgressStore p,AudioPlayer a,ChangeListener l){
        context=c;progress=p;audio=a;bridgeRepo=EnglishBridgeRepository.get(c);memoryAidRepo=MemoryAidRepository.get(c);listener=l;masteryLabels=Arrays.asList(LearningLanguage.ui(p,"0 陌生","0 New"),LearningLanguage.ui(p,"1 见过","1 Seen"),LearningLanguage.ui(p,"2 能认出","2 Recognize"),LearningLanguage.ui(p,"3 能听懂","3 Understand audio"),LearningLanguage.ui(p,"4 能主动说","4 Can produce"),LearningLanguage.ui(p,"5 已掌握","5 Mastered"));setHasStableIds(true);
    }
    ...
    public void submit(List<Word> list){
        ...
        DiffUtil.DiffResult diff=DiffUtil.calculateDiff(new DiffUtil.Callback(){
            ...
            public boolean areContentsTheSame(int oldPos,int newPos){
                ...
                return a.id==b.id&&safe(a.chinese).equals(safe(b.chinese))&&safe(a.english).equals(safe(b.english))&&safe(a.formInfo).equals(safe(b.formInfo));
            }
        },false);
    ...
    @Override public void onBindViewHolder(@NonNull Holder h,int pos){
        ...
        h.word.setText(w.word);h.ipa.setText(safe(w.ipa));h.pron.setText(safe(w.zhPron));h.chinese.setText(LearningLanguage.wordMeaning(w,progress));
        ...
        h.english.setText(safe(w.english).isEmpty()?"":"英文参考："+w.english);h.english.setVisibility(isExpanded&&progress.supportLanguageMode()==ProgressStore.SUPPORT_ZH&&!safe(w.english).isEmpty()?View.VISIBLE:View.GONE);
        ...
        h.pron.setVisibility(progress.shouldShowPronunciation(w.id,isExpanded)?View.VISIBLE:View.GONE);h.detailPanel.setVisibility(isExpanded?View.VISIBLE:View.GONE);h.expandHint.setText(isExpanded?"收起详细信息  ▴":"展开助记、词性、例句与复习  ▾");
    ...
    private void bindDetails(Holder h,Word w){
        ...
        EnglishBridgeEntry bridge=bridgeRepo.forWord(w);h.englishBridge.setVisibility(bridge==null?View.GONE:View.VISIBLE);if(bridge!=null)h.englishBridge.setText(bridge.displayText());
        MemoryAidRepository.MorphologyHint mh=memoryAidRepo.morphologyFor(w);h.memoryMorphology.setVisibility(mh==null?View.GONE:View.VISIBLE);if(mh!=null)h.memoryMorphology.setText(mh.displayText());
        MemoryAidRepository.MemoryChunk mc=memoryAidRepo.chunkFor(w);h.memoryChunk.setVisibility(mc==null?View.GONE:View.VISIBLE);if(mc!=null)h.memoryChunk.setText(mc.displayText());
        h.dimensions.setText("四维掌握  ·  识义 "+progress.meaningLevel(w.id)+"  ·  听力 "+progress.listeningLevel(w.id)+"  ·  拼写 "+progress.spellingLevel(w.id)+"  ·  口语 "+progress.speakingLevel(w.id)+"\n当前弱项："+progress.weakestDimensionName(w.id)+"\n四维复习：识义 "+dimDue(w.id,ProgressStore.DIM_MEANING)+" · 听力 "+dimDue(w.id,ProgressStore.DIM_LISTENING)+" · 拼写 "+dimDue(w.id,ProgressStore.DIM_SPELLING)+" · 口语 "+dimDue(w.id,ProgressStore.DIM_SPEAKING));
        ...
        boolean hasExample=ExampleQuality.isUsable(w);h.examplePanel.setVisibility(hasExample?View.VISIBLE:View.GONE);if(hasExample){h.example.setText(w.example);h.exampleZh.setText(LearningLanguage.sentenceSupport(context,w.example,w.exampleZh,progress));h.exampleAudio.setOnClickListener(v->audio.speak(w.example));}
    ...
    private void clearHeavyDetails(Holder h){
        h.lemma.setVisibility(View.GONE);h.englishBridge.setVisibility(View.GONE);h.memoryMorphology.setVisibility(View.GONE);h.memoryChunk.setVisibility(View.GONE);h.grammar.setVisibility(View.GONE);h.family.setVisibility(View.GONE);h.examplePanel.setVisibility(View.GONE);h.topic.setText("");h.dimensions.setText("");h.reviewInfo.setText("");
    }
    ...
    private String dimDue(int id,int dim){if(progress.dimensionLevel(id,dim)<=0)return "未建立";java.time.LocalDate d=progress.dimensionNextDueDate(id,dim);return d==null?"待安排":(d+"/"+progress.dimensionIntervalDays(id,dim)+"天");}
    private static String safe(String s){return s==null?"":s;}
    @Override public int getItemCount(){return items.size();}

    static class Holder extends RecyclerView.ViewHolder{
        MaterialCardView card;TextView word,ipa,pron,grammarBadge,chinese,english,topic,lemma,englishBridge,memoryMorphology,memoryChunk,grammar,family,dimensions,example,exampleZh,reviewInfo,expandHint;ImageButton audio,favorite,exampleAudio;Spinner mastery;View examplePanel,detailPanel;
        Holder(View v){super(v);card=v.findViewById(R.id.card_word);word=v.findViewById(R.id.text_word);ipa=v.findViewById(R.id.text_ipa);pron=v.findViewById(R.id.text_pron);grammarBadge=v.findViewById(R.id.text_grammar_badge);chinese=v.findViewById(R.id.text_chinese);english=v.findViewById(R.id.text_english);topic=v.findViewById(R.id.text_topic);lemma=v.findViewById(R.id.text_lemma);englishBridge=v.findViewById(R.id.text_english_bridge);memoryMorphology=v.findViewById(R.id.text_memory_morphology);memoryChunk=v.findViewById(R.id.text_memory_chunk);grammar=v.findViewById(R.id.text_grammar);family=v.findViewById(R.id.text_family);dimensions=v.findViewById(R.id.text_dimensions);example=v.findViewById(R.id.text_example);exampleZh=v.findViewById(R.id.text_example_zh);examplePanel=v.findViewById(R.id.example_panel);detailPanel=v.findViewById(R.id.detail_panel);expandHint=v.findViewById(R.id.text_expand_hint);audio=v.findViewById(R.id.button_audio);favorite=v.findViewById(R.id.button_favorite);exampleAudio=v.findViewById(R.id.button_example_audio);reviewInfo=v.findViewById(R.id.text_review_info);mastery=v.findViewById(R.id.spinner_mastery);}
    }
}
