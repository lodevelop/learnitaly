...
public final class WeeklyExamEngine {
    ...
    private Item wordChoice(String id,String skill,String label,Word target,List<Word> pool,boolean listening){
        ...
        q.expected=listening?target.word:LearningLanguage.wordMeaning(target,progress);
        q.prompt=listening?LearningLanguage.ui(progress,"🔊 听音，选择你听到的意大利语单词","🔊 Listen and choose the Italian word"):LearningLanguage.ui(progress,"选择正确词义：\n","Choose the correct meaning:\n")+target.word;
        q.hint=listening?LearningLanguage.ui(progress,"只听声音，不显示词义提示","Listen only; no meaning hint is shown"):LearningLanguage.ui(progress,"不要凭词形猜，先主动回忆词义","Recall the meaning before guessing from the spelling");
        q.choices.add(q.expected);int guard=0;while(q.choices.size()<4&&guard++<300){Word w=pool.get(random.nextInt(pool.size()));String x=listening?w.word:LearningLanguage.wordMeaning(w,progress);if(x!=null&&!x.trim().isEmpty()&&!q.choices.contains(x))q.choices.add(x);}Collections.shuffle(q.choices,random);return q;
    }

    private Item spelling(String id,Word w){Item q=new Item();q.id=id;q.skillKey=MasteryPassportEngine.ACTION_SPELLING;q.skillLabel=LearningLanguage.ui(progress,"拼写","Spelling");q.type=TYPE_TYPED;q.word=w;q.prompt=LearningLanguage.ui(progress,"根据词义写出意大利语：\n","Write the Italian from the meaning:\n")+LearningLanguage.wordMeaning(w,progress);q.expected=w.word;q.hint=LearningLanguage.ui(progress,"不给首字母，不给选项","No first letter and no choices");return q;}

    private Item sentence(String id,String skill,String label,CoreSentence s,boolean speaking){Item q=new Item();q.id=id;q.skillKey=skill;q.skillLabel=LearningLanguage.ui(progress,label,skillLabelEn(skill));q.type=speaking?TYPE_SPEAK:TYPE_TYPED;String support=LearningLanguage.sentenceSupport(words,s.italian,s.chinese,progress);q.prompt=(speaking?LearningLanguage.ui(progress,"根据提示说出完整意大利语：\n","Say the complete Italian sentence from the prompt:\n"):LearningLanguage.ui(progress,"生活场景 · "+safeCategory(s.category)+"\n请用意大利语表达：\n","Real-life scenario\nExpress this in Italian:\n"))+support;q.expected=s.italian;q.hint=speaking?LearningLanguage.ui(progress,"可以用麦克风；语音识别不可用时可输入","Use the microphone, or type if speech recognition is unavailable"):LearningLanguage.ui(progress,"允许自然表达；本地匹配会重点检查核心词和句子完整度","Natural wording is allowed; local matching focuses on key words and completeness");return q;}
    ...
    private String safeChinese(Word w){return LearningLanguage.wordMeaning(w,progress);}
    private String skillLabelEn(String key){if(MasteryPassportEngine.ACTION_LISTENING.equals(key))return "Listening";if(MasteryPassportEngine.ACTION_SPELLING.equals(key))return "Spelling";if(MasteryPassportEngine.ACTION_SPEAKING.equals(key))return "Speaking";if(MasteryPassportEngine.ACTION_GRAMMAR.equals(key))return "Grammar";if(MasteryPassportEngine.ACTION_REAL_USE.equals(key))return "Real-life use";return "Meaning";}
