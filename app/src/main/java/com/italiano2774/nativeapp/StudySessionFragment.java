...
public class StudySessionFragment extends Fragment {
    ...
    private void configurePrompt(Word w){
        ...
        switch(currentMode){
            case SmartReviewModeEngine.MODE_ZH_IT:
                word.setTextSize(28);word.setText(LearningLanguage.recallPrompt(w,progress));ipa.setText("先在脑中说出意大利语"+(isNoun(w)?"，名词请连冠词一起想":"")+"，再显示答案");sessionAudio.setVisibility(View.GONE);showAnswer.setText("显示意大利语答案");break;
            ...
            case SmartReviewModeEngine.MODE_CLOZE:
                word.setTextSize(25);word.setText(SmartReviewModeEngine.clozeSentence(w));ipa.setText((w.exampleZh==null||w.exampleZh.trim().isEmpty())?LearningLanguage.ui(progress,"选择最适合填入句子的词","Choose the best word for the sentence"):LearningLanguage.sentenceSupport(requireContext(),w.example,w.exampleZh,progress));sessionAudio.setVisibility(View.GONE);showAnswer.setText("想不起来 · 显示答案");setupChoices(w);break;
            default:
    ...
    private void populateAnswerData(Word w){
        answerWord.setText(channelReview()?SmartReviewModeEngine.studyForm(w):w.word);pron.setText(w.zhPron==null?"":w.zhPron);pron.setVisibility(progress.shouldShowPronunciation(w.id,false)?View.VISIBLE:View.GONE);chinese.setText(LearningLanguage.wordMeaning(w,progress));english.setText(w.english==null||w.english.trim().isEmpty()?"":"英文参考："+w.english);english.setVisibility(progress.supportLanguageMode()==ProgressStore.SUPPORT_ZH&&w.english!=null&&!w.english.trim().isEmpty()?View.VISIBLE:View.GONE);
        ...
        boolean hasExample=ExampleQuality.isUsable(w);examplePanel.setVisibility(hasExample?View.VISIBLE:View.GONE);if(hasExample){example.setText(w.example);exampleZh.setText(LearningLanguage.sentenceSupport(requireContext(),w.example,w.exampleZh,progress));}
    }
