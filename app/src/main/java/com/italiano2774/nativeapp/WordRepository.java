...
public class WordRepository {
    ...
    private final Map<String,Integer> chinesePromptCount=new HashMap<>();
    private final Map<String,Integer> englishPromptCount=new HashMap<>();
    ...
    private WordRepository(Context context){
        try{
            ...
            for(int i=0;i<arr.length();i++){
                ...
                String zhKey=normalizeChinesePrompt(w.chinese);if(!zhKey.isEmpty())chinesePromptCount.put(zhKey,chinesePromptCount.getOrDefault(zhKey,0)+1);String enKey=normalizeEnglishPrompt(w.english);if(!enKey.isEmpty())englishPromptCount.put(enKey,englishPromptCount.getOrDefault(enKey,0)+1);
    ...
    private static String normalizeChinesePrompt(String value){return value==null?"":value.trim().replaceAll("\\s+","");}
    private static String normalizeEnglishPrompt(String value){if(value==null)return "";String v=value.trim().toLowerCase(java.util.Locale.ROOT);int bar=v.indexOf('|');if(bar>=0)v=v.substring(0,bar);return v.replaceAll("[^a-z0-9]+"," ").trim();}
    /** Reverse recall only uses Chinese prompts that identify one shipped headword unambiguously. */
    public boolean isChinesePromptUnique(Word w){if(w==null)return false;String key=normalizeChinesePrompt(w.chinese);return !key.isEmpty()&&chinesePromptCount.getOrDefault(key,0)==1;}
    public boolean isEnglishPromptUnique(Word w){if(w==null)return false;String key=normalizeEnglishPrompt(w.english);return !key.isEmpty()&&englishPromptCount.getOrDefault(key,0)==1;}
    public boolean isSupportPromptUnique(Word w,ProgressStore progress){int mode=progress==null?ProgressStore.SUPPORT_BOTH:progress.supportLanguageMode();if(mode==ProgressStore.SUPPORT_EN)return isEnglishPromptUnique(w);if(mode==ProgressStore.SUPPORT_BOTH)return isChinesePromptUnique(w)&&isEnglishPromptUnique(w);return isChinesePromptUnique(w);}
    ...
    public DailyPlan smartMemoryPlan(ProgressStore progress){
        ...
        int unknown=0;for(int i=0;i<routeLimit;i++)if(progress.mastery(words.get(i).id)==0)unknown++;plan.newQuota=Math.min(progress.recommendedNewWords(words),unknown);Set<Integer> recent=progress.recentWordIds();
