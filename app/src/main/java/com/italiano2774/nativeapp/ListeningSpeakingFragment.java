...
public class ListeningSpeakingFragment extends Fragment {
    ...
    private void showSentenceListening(){modeText.setText("听一句 · 选择中文意思");prompt.setText("🔊 先听，不看字幕");subtitle.setText(currentSentence.source+" · 可重复听，也可以用慢速");choices.setVisibility(View.VISIBLE);speaking.setVisibility(View.GONE);audioControls.setVisibility(View.VISIBLE);List<String> opts=new ArrayList<>();opts.add(LearningLanguage.sentenceSupport(words,currentSentence.italian,currentSentence.chinese,progress));while(opts.size()<4){SentenceCard other=sentences.get(random.nextInt(sentences.size()));String z=LearningLanguage.sentenceSupport(words,other.italian,other.chinese,progress);if(!opts.contains(z))opts.add(z);}Collections.shuffle(opts,random);correctOption=LearningLanguage.sentenceSupport(words,currentSentence.italian,currentSentence.chinese,progress);bindOptions(opts);prompt.postDelayed(()->{if(isAdded()&&!answered&&currentMode==MODE_SENTENCE_LISTEN)audio.speak(currentSentence.italian,1.0f);},220);}
    ...
    private void showSpeaking(){choices.setVisibility(View.GONE);speaking.setVisibility(View.VISIBLE);if(currentMode==MODE_REPEAT){modeText.setText("跟读句子 · 听后模仿");prompt.setText(currentSentence.italian);subtitle.setText(LearningLanguage.sentenceSupport(words,currentSentence.italian,currentSentence.chinese,progress)+"\n"+LearningLanguage.ui(progress,"先听原句，再完整说一遍；系统语音识别用于内容匹配，不代替人工发音评分。","Listen first, then repeat the full sentence. Speech recognition checks content, not accent quality."));audioControls.setVisibility(View.VISIBLE);prompt.postDelayed(()->{if(isAdded()&&!answered&&currentMode==MODE_REPEAT)audio.speak(currentSentence.italian,1.0f);},220);}else{modeText.setText("主动开口 · 看中文说意大利语");prompt.setText(LearningLanguage.sentenceSupport(words,currentSentence.italian,currentSentence.chinese,progress));subtitle.setText("先自己说，不看意大利语。说完后系统会显示识别结果和标准句。");audioControls.setVisibility(View.GONE);}speech.setText("🎙 说出来并评分");}
