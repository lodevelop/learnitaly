...
for f in ['CourseUnit.java','CourseCurriculumRepository.java','CourseQuestion.java','CourseDistributionPlanner.java','CourseLessonEngine.java','CourseHomeFragment.java','CourseMapFragment.java','CourseLessonFragment.java','ProfileFragment.java','ExampleQuality.java']:
    req(ROOT/'app/src/main/java/com/italiano2774/nativeapp'/f)
...
req(ROOT/'tools/course_distribution_quality_check.py')
...
req(ROOT/'tools/bilingual_support_quality_check.py')
req(ROOT/'tools/ui_language_integrity_quality_check.py')
...
if "versionName '5.4.1-native'" not in build_text: errors.append('v5.4.1 versionName missing or app/build.gradle was not fully overwritten')
if 'def defaultVersionCode = 89' not in build_text or 'versionCode resolvedVersionCode' not in build_text: errors.append('v5.4.1 dynamic/fallback versionCode configuration missing')
if "applicationId 'com.italiano2774.nativeapp'" not in build_text: errors.append('permanent update package id changed')
if 'signingConfig signingConfigs.release' not in build_text or 'CM_KEYSTORE_PATH' not in build_text: errors.append('v5.4.1 permanent release signing config missing')
