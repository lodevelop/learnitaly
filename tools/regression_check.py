...
required=[
    ...
    'tools/bilingual_support_quality_check.py',
    'tools/ui_language_integrity_quality_check.py',
...
checks={
 ...
 'upgrade snapshot':'5.4.1-preupgrade' in java and 'ensureVersionBackupThen' in java,
...
if "versionName '5.4.1-native'" not in build: err('versionName is not 4.9.0-native')
if 'def defaultVersionCode = 89' not in build or 'versionCode resolvedVersionCode' not in build: err('v5.4.1 versionCode update-chain config missing')
...
if 'v5.4.1' not in cm or 'android_signing:' not in cm or '- zhongxue_release' not in cm or ':app:assembleRelease' not in cm or 'app-release.apk' not in cm: err('v5.4.1 Codemagic signed-release update chain missing')
...
if 'ACTION_REAL_USE' not in passport or 'checkpoint("A1"' not in passport or 'checkpoint("B1"' not in passport: err('v5.4.1 mastery passport engine missing')
if 'openMasteryPassport' not in main_for_passport or 'bindCheckpoint' not in passport_ui: err('v5.4.1 mastery passport navigation/UI missing')
breakthrough=text(ROOT/'app/src/main/java/com/italiano2774/nativeapp/BreakthroughPlanEngine.java')
if 'threeDayText' not in breakthrough or 'todayTask' not in breakthrough or 'focusSkillKey' not in passport: err('v5.4.1 three-day breakthrough engine missing')
...
if 'reviewDue(' in engine or 'addDueReview' in engine: err('v5.4.1 fixed course still injects cross-unit due-review words')
if 'addSeenReinforcement' not in engine: err('v5.4.1 same-unit course reinforcement missing')
if 'CourseDistributionPlanner.focusIds' not in engine or 'CourseDistributionPlanner.challengeIds' not in engine: err('v5.4.1 balanced course distribution planner not wired into lesson engine')
if 'python3 tools/course_distribution_quality_check.py' not in cm: err('v5.4.1 course distribution quality check missing')
...
if 'dimensionDueEpochDay' not in smart_store or 'priorityReviewDimension' not in smart_store or 'forgettingFactor' not in smart_store: err('v5.4.1 four-track forgetting scheduler missing')
if 'channelReview()' not in smart_session or 'dimensionNextDueDate' not in smart_session: err('v5.4.1 due-dimension review routing missing')
...
if 'coachLine' not in beginner_guide or 'practiceUnlocked' not in beginner_guide: err('v5.4.1 beginner guide engine missing')
if 'button_home_details_toggle' not in beginner_home or 'progress.homeSimpleMode()' not in beginner_home: err('v5.4.1 compact home/detail toggle missing')
if 'bindGuided' not in beginner_hub or 'BeginnerGuideEngine.active' not in beginner_hub: err('v5.4.1 progressive practice unlock missing')
...
if 'python3 tools/course_unit_isolation_quality_check.py' not in cm: err('v5.4.1 fixed-course unit isolation quality check missing')
...
if 'python3 tools/listening_speaking_quality_check.py' not in cm: err('v5.4.1 listening/speaking bridge quality check missing')
if 'python3 tools/scenario_conversation_quality_check.py' not in cm: err('v5.4.1 scenario difficulty quality check missing')
if 'python3 tools/micro_grammar_quality_check.py' not in cm: err('v5.4.1 micro grammar quality check missing')
if 'python3 tools/daily_smart_plan_quality_check.py' not in cm: err('v5.4.1 daily smart plan quality check missing')
if 'python3 tools/daily_speaking_quality_check.py' not in cm: err('v5.4.1 daily five-sentence speaking quality check missing')
if 'python3 tools/mastery_passport_quality_check.py' not in cm: err('v5.4.1 mastery passport quality check missing')
if 'python3 tools/breakthrough_plan_quality_check.py' not in cm: err('v5.4.1 breakthrough prescription quality check missing')
if 'python3 tools/daily_learning_loop_quality_check.py' not in cm: err('v5.4.1 daily learning-loop quality check missing')
if 'python3 tools/review_forecast_quality_check.py' not in cm: err('v5.4.1 seven-day consolidation forecast quality check missing')
...
if 'pathSummary' not in smart_home or '听、说、读、写会由系统自动穿插' not in smart_home: err('v5.4.1 compact course-home mixed-skill explanation missing')
...
if 'markMatchingErrorRepaired' not in err_frag or 'unresolvedPracticeErrors' not in err_dao: err('v5.4.1 personal error-evidence repair loop missing')
if 'version = 6' not in err_db or 'MIGRATION_5_6' not in err_db: err('v5.4.1 Room v6 repair-state migration missing')
if 'python3 tools/error_evidence_quality_check.py' not in cm: err('v5.4.1 error-evidence Codemagic gate missing')
if 'python3 tools/weekly_exam_quality_check.py' not in cm: err('v5.4.1 weekly practical-exam Codemagic gate missing')
if 'python3 tools/weak_word_story_quality_check.py' not in cm: err('v5.4.1 weak-word micro-reading Codemagic gate missing')
if 'python3 tools/life_task_map_quality_check.py' not in cm: err('v5.4.1 real-life task-map Codemagic gate missing')
if 'python3 tools/personal_forgetting_model_quality_check.py' not in cm: err('v5.4.1 personal forgetting model Codemagic gate missing')
if 'python3 tools/beginner_home_quality_check.py' not in cm: err('v5.4.1 beginner-first home Codemagic gate missing')
if 'python3 tools/bilingual_support_quality_check.py' not in cm: err('v5.4.1 bilingual support Codemagic gate missing')
if 'python3 tools/ui_language_integrity_quality_check.py' not in cm: err('v5.4.1 whole-sentence English UI integrity Codemagic gate missing')
...
if 'rankedWeakWords' not in weak_story_engine or 'fillFromExamples' not in weak_story_engine or 'buildClozeTargets' not in weak_story_engine: err('v5.4.1 weak-word micro-reading engine missing')
if 'recordAuxiliaryResult("weak_story"' not in weak_story_fragment or 'recordEmbeddedDimensionResults' not in weak_story_fragment or 'score>=60' not in weak_story_fragment: err('v5.4.1 weak-word read/listen/cloze/retell feedback loop missing')
...
if life_repo.count('new LifeTask(')!=12: err('v5.4.1 task map must define 12 real-life missions')
for _m in ['passLine(int level)','completedStages(ProgressStore p)','nextTask(ProgressStore p)','unlocked(ProgressStore p,LifeTask task,int level)']:
    if _m not in life_engine: err('v5.4.1 task progression engine missing '+_m)
for _m in ['LifeTaskEngine.unlocked','openLifeTaskStage','先通过上一关']:
    if _m not in life_detail: err('v5.4.1 sequential task unlock missing '+_m)
if 'recordAuxiliaryResult("life_task"' not in life_dialogue or 'newInstance(String id,int level,String lifeTaskId)' not in life_dialogue: err('v5.4.1 life-task dialogue result bridge missing')
if 'LifeTaskEngine.nextTask(progress)' not in life_plan or '"life_task"' not in life_plan: err('v5.4.1 daily/weekly life-task scheduling missing')
...
for _m in ['TOTAL_QUESTIONS=18','QUESTIONS_PER_SKILL=3','ACTION_REAL_USE']:
    if _m not in weekly_engine: err('v5.4.1 weekly exam engine missing '+_m)
for _m in ['SpeechRecognizer','saveWeeklyExamResult','recordAuxiliaryResult("weekly_exam"']:
    if _m not in weekly_fragment: err('v5.4.1 weekly exam feedback loop missing '+_m)
for _m in ['weeklyExamDue()','weeklyAdjustmentSummary()','weeklyNewWordPercent()']:
    if _m not in weekly_store: err('v5.4.1 weekly diagnosis persistence missing '+_m)
for _m in ['weeklyFocusTask','weekly_exam','addIfFitsUniqueAction']:
    if _m not in weekly_plan: err('v5.4.1 weekly diagnosis daily scheduling missing '+_m)
