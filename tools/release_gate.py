...
for needle,msg in [
    ('def defaultVersionCode = 88','v5.4.0 local fallback versionCode must be 83'),
...
if 'python3 tools/course_unit_isolation_quality_check.py' not in cm: error('Codemagic v5.4.0 fixed-course unit isolation check step missing')
if 'python3 tools/course_distribution_quality_check.py' not in cm: error('Codemagic v5.4.0 fixed-course distribution audit step missing')
...
if 'python3 tools/listening_speaking_quality_check.py' not in cm: error('Codemagic v5.4.0 listening/speaking bridge check step missing')
if 'python3 tools/scenario_conversation_quality_check.py' not in cm: error('Codemagic v5.4.0 three-level scenario conversation check step missing')
if 'python3 tools/micro_grammar_quality_check.py' not in cm: error('Codemagic v5.4.0 micro grammar check step missing')
if 'python3 tools/daily_smart_plan_quality_check.py' not in cm: error('Codemagic v5.4.0 daily smart plan check step missing')
if 'python3 tools/daily_speaking_quality_check.py' not in cm: error('Codemagic v5.4.0 daily five-sentence speaking check step missing')
if 'python3 tools/mastery_passport_quality_check.py' not in cm: error('Codemagic v5.4.0 mastery passport check step missing')
if 'python3 tools/breakthrough_plan_quality_check.py' not in cm: error('Codemagic v5.4.0 breakthrough prescription check step missing')
if 'python3 tools/error_evidence_quality_check.py' not in cm: error('Codemagic v5.4.0 error evidence repair check step missing')
if 'python3 tools/error_prompt_integrity_quality_check.py' not in cm: error('Codemagic v5.4.0 personal error prompt-integrity check step missing')
if 'python3 tools/weekly_exam_quality_check.py' not in cm: error('Codemagic v5.4.0 weekly practical exam check step missing')
if 'python3 tools/shadowing_training_quality_check.py' not in cm: error('Codemagic v5.4.0 Shadowing training-room check step missing')
if 'python3 tools/weak_word_story_quality_check.py' not in cm: error('Codemagic v5.4.0 weak-word micro-reading check step missing')
if 'python3 tools/life_task_map_quality_check.py' not in cm: error('Codemagic v5.4.0 real-life task-map check step missing')
if 'python3 tools/personal_forgetting_model_quality_check.py' not in cm: error('Codemagic v5.4.0 personal forgetting model check step missing')
if 'python3 tools/beginner_home_quality_check.py' not in cm: error('Codemagic v5.4.0 beginner-first home check step missing')
if 'python3 tools/bilingual_support_quality_check.py' not in cm: error('Codemagic v5.4.0 bilingual support check step missing')
if 'python3 tools/ui_language_integrity_quality_check.py' not in cm: error('Codemagic v5.4.0 whole-sentence English UI integrity check step missing')
if 'python3 tools/daily_learning_loop_quality_check.py' not in cm: error('Codemagic v5.4.0 daily learning-loop check step missing')
if 'python3 tools/review_forecast_quality_check.py' not in cm: error('Codemagic v5.4.0 seven-day consolidation forecast check step missing')
if 'python3 tools/release_gate.py' not in cm: error('Codemagic strict release gate step missing')
if 'v5.4.0' not in cm: error('Codemagic workflow title must identify v5.4.0')
...
if 'reviewDue(' in course_engine or 'addDueReview' in course_engine: error('v5.4.0 fixed course still injects global due-review words')
if 'addSeenReinforcement' not in course_engine: error('v5.4.0 same-unit reinforcement missing')
course_planner=read(JAVA/'CourseDistributionPlanner.java')
if 'CourseDistributionPlanner.focusIds' not in course_engine or 'CourseDistributionPlanner.challengeIds' not in course_engine: error('v5.4.0 balanced exposure planner not wired into fixed lessons')
for _m in ['reinforcementBudget','first<=lesson-2','scheduledExposureCount']:
    if _m not in course_planner: error('v5.4.0 course distribution planner missing '+_m)
...
if 'pathSummary' not in course_home or '听、说、读、写会由系统自动穿插' not in course_home: error('v5.4.0 compact mixed-skill course-home explanation missing')
...
if 'renderDailyPlan' not in course_home or 'openDailyTask' not in course_home or '开始下一项' not in course_home: error('v5.4.0 daily smart-plan home flow missing')
if 'case "error_repair"' not in course_home or '查看今日总结 · 明日复习预告' not in course_home: error('v5.4.0 recovery/summary close-loop home flow missing')
if 'BeginnerGuideEngine.coachLine' not in course_home or 'button_home_details_toggle' not in course_home or 'progress.homeSimpleMode()' not in course_home: error('v5.4.0 beginner-first compact home flow missing')
beginner_guide=read(JAVA/'BeginnerGuideEngine.java')
practice_hub=read(JAVA/'PracticeHubFragment.java')
if 'practiceUnlocked' not in beginner_guide or '第7天' not in beginner_guide: error('v5.4.0 seven-day beginner guide missing')
if 'bindGuided' not in practice_hub or 'BeginnerGuideEngine.active' not in practice_hub: error('v5.4.0 progressive practice unlock missing')
...
if 'carryAfter' not in review_forecast or 'baseCapacity' not in review_forecast or 'remainingAfterWindow' not in review_forecast: error('v5.4.0 seven-day review forecast engine missing')
if 'renderReviewForecast' not in calendar_java or 'layout_review_forecast' not in calendar_java: error('v5.4.0 consolidation calendar rendering missing')
if 'plan.newQuota=Math.min(progress.recommendedNewWords(words),unknown)' not in word_repo: error('v5.4.0 smart-memory session ignores adaptive new-word quota')
if 'tomorrowScheduled' not in forecast_store or 'tomorrowHeadroom' not in forecast_store or 'forecastPressure' not in forecast_store: error('v5.4.0 future-load new-word throttle missing')
...
for _m in ['ACTION_MEANING','ACTION_LISTENING','ACTION_SPELLING','ACTION_SPEAKING','ACTION_GRAMMAR','ACTION_REAL_USE','checkpoint("A1"','checkpoint("A2"','checkpoint("B1"']:
    if _m not in mastery_engine: error('v5.4.0 mastery passport engine missing '+_m)
for _m in ['bindSkill','bindCheckpoint','openAction']:
    if _m not in mastery_fragment: error('v5.4.0 mastery passport fragment missing '+_m)
if '不是官方CEFR考试或证书结论' not in mastery_layout: error('v5.4.0 mastery passport CEFR disclaimer missing')
...
for _m in ['threeDayText','todayTask','sentence_dictation','daily_speaking','ACTION_REAL_USE']:
    if _m not in breakthrough_engine: error('v5.4.0 breakthrough prescription engine missing '+_m)
if 'BreakthroughPlanEngine.todayTask' not in read(APP/'src/main/java/com/italiano2774/nativeapp/DailySmartPlanEngine.java'): error('v5.4.0 daily planner does not consume passport prescription')
...
for _m in ['unresolvedPracticeErrors(40)','markMatchingErrorRepaired','markEvidenceRepairComplete','SentenceFsrsRepository.recordDimension']:
    if _m not in error_evidence: error('v5.4.0 error evidence repair loop missing '+_m)
if 'case "error_evidence_repair"' not in course_home: error('v5.4.0 daily planner route for personal error notebook missing')
...
for _m in ['TOTAL_QUESTIONS=18','QUESTIONS_PER_SKILL=3','ACTION_MEANING','ACTION_REAL_USE']:
    if _m not in weekly_engine: error('v5.4.0 weekly exam engine missing '+_m)
for _m in ['SpeechRecognizer','saveWeeklyExamResult','weekly_exam']:
    if _m not in weekly_fragment: error('v5.4.0 weekly exam result/feedback missing '+_m)
for _m in ['weeklyExamDue','weeklyFocusTask','weekly_exam','addIfFitsUniqueAction']:
    if _m not in weekly_plan: error('v5.4.0 weekly diagnosis planner integration missing '+_m)
if 'case "weekly_exam"' not in course_home: error('v5.4.0 daily planner weekly-exam route missing')
...
for _m in ['rankedWeakWords','rankSections','fillFromSection','fillFromExamples','buildClozeTargets']:
    if _m not in weak_story_engine: error('v5.4.0 weak-word story engine missing '+_m)
for _m in ['第1 / 4步','第2 / 4步','第3 / 4步','第4 / 4步','recordEmbeddedDimensionResults','recordAuxiliaryResult("weak_story"','SentenceFsrsRepository']:
    if _m not in weak_story_fragment: error('v5.4.0 weak-word four-stage loop missing '+_m)
if 'button_weak_story_mic' not in weak_story_layout or '不联网生成' not in weak_story_layout: error('v5.4.0 weak-word story layout/disclosure missing')
...
for _m in ['new LifeTask(','clothing_store','post_office','phone_appointment','bureaucracy']:
    if _m not in life_repo: error('v5.4.0 real-life task repository missing '+_m)
for _m in ['passLine(int level)','completedStages(ProgressStore p)','nextTask(ProgressStore p)','unlocked(ProgressStore p,LifeTask task,int level)']:
    if _m not in life_engine: error('v5.4.0 36-stage task progression missing '+_m)
for _m in ['LifeTaskEngine.unlocked','openLifeTaskStage','先通过上一关']:
    if _m not in life_detail: error('v5.4.0 task detail sequential unlock missing '+_m)
for _m in ['newInstance(String id,int level,String lifeTaskId)','recordAuxiliaryResult("life_task"','LifeTaskEngine.passLine(currentLevel)']:
    if _m not in dialogue_training: error('v5.4.0 task/dialogue result bridge missing '+_m)
if 'case "life_task"' not in course_home: error('v5.4.0 daily plan life-task route missing')
if 'LifeTaskEngine.nextTask(progress)' not in weekly_plan or 'dailyAuxiliaryCorrect("life_task"' not in weekly_plan: error('v5.4.0 daily/weekly task-map scheduling missing')
if '"life_task"' not in mastery_engine: error('v5.4.0 life-task evidence missing from mastery passport')
...
for _m in ['PASS_LINE={60,72,82}','MediaRecorder','MediaPlayer','SpeechRecognizer','第2遍 · 只看中文主动说','第3遍 · 裸说迁移','SentenceFsrsRepository.DIM_SPEAKING','recordErrorCause(ErrorCause.PRONUNCIATION','shadowing_pass']:
    if _m not in shadowing: error('v5.4.0 Shadowing training room missing '+_m)
for _m in ['button_shadow_record_play','button_shadow_speech','button_speed_mid','Shadowing 三遍训练室']:
    if _m not in shadow_layout: error('v5.4.0 Shadowing layout missing '+_m)
if '口语突破 · 第2天' not in breakthrough_engine or '"shadow",""' not in breakthrough_engine: error('v5.4.0 three-day speaking prescription does not route day 2 to Shadowing')
...
for _m in ['learnedDimensionPct','dueCount(all,today.plusDays(1))','button_summary_repair','tomorrowAdvice']:
    if _m not in daily_summary: error('v5.4.0 daily summary close-loop missing '+_m)
...
if 'openListeningSpeaking' not in main_nav: error('v5.4.0 MainActivity listening/speaking navigation missing')
for _m in ['openMicroGrammar','openMicroGrammarLesson']:
    if _m not in main_nav: error('v5.4.0 MainActivity micro-grammar navigation missing '+_m)
...
for _m in ['今天先学：','20秒看懂 · 做3题','到期复习','基本稳定','openMicroGrammarLesson']:
    if _m not in micro_grammar: error('v5.4.0 micro-grammar route missing '+_m)
...
for _m in ['SpeechRecognizer','MediaRecorder','MODE_SENTENCE_LISTEN','MODE_WORD_LISTEN','MODE_REPEAT','MODE_ZH_SPEAK','listen_speak','openMemoryArticleReview']:
    if _m not in listen_speak: error('v5.4.0 listening/speaking bridge missing '+_m)
dialogue_training=read(APP/'src/main/java/com/italiano2774/nativeapp/DialogueTrainingFragment.java')
for _m in ['LEVEL_BEGINNER','LEVEL_INTERMEDIATE','LEVEL_ADVANCED','FreeConversationEngine','dialogueScenarioRecommendedLevel','markDialogueScenarioCompletion(current.id,score,currentLevel)']:
    if _m not in dialogue_training: error('v5.4.0 three-level scenario flow missing '+_m)
...
if 'dimensionDueEpochDay' not in smart_store or 'priorityReviewDimension' not in smart_store or 'forgettingFactor' not in smart_store: error('v5.4.0 four-track forgetting scheduler missing')
if 'channelReview()' not in smart_session or 'dimensionNextDueDate' not in smart_session: error('v5.4.0 due-dimension review routing missing')
